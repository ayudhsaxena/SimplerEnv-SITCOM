...
class LatentActionQuantizationPretrainedInit(nn.Module):
    def __init__(
        self,
        *,
        dim,
    ...
    def encode(
        ...
        last_video_tokens = video_tokens[:, 1:]
        first_tokens = tokens[:, :1]
        last_tokens = tokens[:, 1:]
        
        return first_tokens, last_tokens, first_video_tokens, last_video_tokens
    ...
    def forward(
        ...
        *_, h, w, _ = shape
        ...
        # TODO: check if we need to detach first_frame_tokens
        if self.use_spatial_feat_condition:
            concat_tokens = rearrange(first_tokens, 'b (t h w) d -> b t h w d', h = h, w = w)
        else:
            concat_tokens = first_frame_tokens #.detach() #+ tokens  [b,t,h,w,d]
