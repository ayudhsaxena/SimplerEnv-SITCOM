...
class NSVQ(torch.nn.Module):
    ...
    def encode(self, input_data, batch_size):
        ...
        input_data = self.project_in(input_data) # [b, f*h*w, d]
        num_patches = int(self.image_size/self.patch_size) * int(self.image_size/self.patch_size)
        num_frames = input_data.shape[1] // num_patches
        input_data = input_data.reshape(batch_size * num_frames, num_patches, self.embedding_dim)  #[b*f, h*w, d]
        input_data = input_data.permute(0, 2, 1).contiguous()
        # reshape input_data for CNN [b*f, d, h, w]
        input_data = input_data.reshape(batch_size * num_frames, self.embedding_dim, int(self.image_size/self.patch_size), int(self.image_size/self.patch_size))
        input_data = self.cnn_encoder(input_data)  # [b*f, d, q, q]
        input_data = input_data.reshape(batch_size * num_frames, self.embedding_dim, -1)  # [b*f, d, q^2]
        input_data = input_data.permute(0, 2, 1).contiguous()  # [b*f, q^2, d]
        input_data = input_data.reshape(-1, self.embedding_dim)  # [b*f*q^2, d]
        return input_data
    
    def decode(self, quantized_input, batch_size):
        # num_frames = quantized_input.shape[0] // batch_size
        quantized_input = quantized_input.reshape(batch_size, -1, self.embedding_dim)  # [b, f*q^2, d]
        quantized_input = self.project_out(quantized_input)
        return quantized_input
    
    def forward(self, input_data_first, input_data_last, mask_last=None, codebook_training_only=False, condn_first=True, inference=False):
        ...
        if inference:
            hard_quantized_input = self.decode(hard_quantized_input, batch_size)
            return hard_quantized_input, min_indices.reshape(batch_size, -1)
    
        random_vector = normal_dist.Normal(0, 1).sample(input_data.shape).to(self.codebooks.device)
        ...
        if mask_last is not None:
            mask_last = repeat(mask_last, 'b f -> b f n', n=self.code_seq_len)
            mask_last = mask_last.reshape(-1)
            avg_probs = torch.mean(encodings[mask_last], dim=0)
        else:
        ...
        with torch.no_grad():
            if mask_last is not None:
                min_indices_cpu = min_indices[mask_last].cpu()
            else:
                min_indices_cpu = min_indices.cpu()
            self.codebooks_used[min_indices_cpu] += 1
            num_unique_indices = min_indices_cpu.unique().size(0)
        ...
        return quantized_input, perplexity, self.codebooks_used.cpu().numpy(), min_indices.reshape(batch_size, -1), num_unique_indices
    ...
    def replace_unused_codebooks_sync(self, num_batches):
        ...
        with torch.no_grad():
            ...
            if dist.is_initialized():
                dist.all_reduce(self.codebooks_used, op=dist.ReduceOp.SUM)
                all_codebooks_used = self.codebooks_used.float() / dist.get_world_size()
                print(f'Rank:{dist.get_rank()} ####### all_codebooks_used: {all_codebooks_used} ######')

            unused_indices = torch.where((all_codebooks_used / num_batches) < self.discarding_threshold)[0]
            used_indices = torch.where((all_codebooks_used / num_batches) >= self.discarding_threshold)[0]

            unused_count = unused_indices.shape[0]
            used_count = used_indices.shape[0]

            if used_count == 0:
                print(f'Rank:{dist.get_rank()} ####### used_indices equals zero / shuffling whole codebooks ######')
                # Ensure deterministic noise for all ranks
                if dist.get_rank() == 0:
                    noise = torch.randn(self.codebooks.size(), device=self.codebooks.device)
                else:
                    noise = torch.zeros(self.codebooks.size(), device=self.codebooks.device)
                # Broadcast noise from rank 0 to all other ranks
                if dist.is_initialized():
                    dist.broadcast(noise, src=0)
                self.codebooks += self.eps * noise.clone()  # Apply synchronized noise
            else:
                used = self.codebooks[used_indices].clone()
                if used_count < unused_count:
                    used_codebooks = used.repeat(int((unused_count / (used_count + self.eps)) + 1), 1)
                    used_codebooks = used_codebooks[torch.randperm(used_codebooks.shape[0])]
                    # Broadcast used codebooks from rank 0 to all other ranks
                    if dist.is_initialized():
                        dist.broadcast(used_codebooks, src=0)
                else:
                    used_codebooks = used

                # Ensure deterministic noise generation
                if dist.get_rank() == 0:
                    noise = torch.randn((unused_count, self.embedding_dim), device=self.codebooks.device)
                else:
                    noise = torch.zeros((unused_count, self.embedding_dim), device=self.codebooks.device)
                ...
                self.codebooks[unused_indices] *= 0
                self.codebooks[unused_indices] += used_codebooks[range(unused_count)] + self.eps * noise.clone()

            print(f'Rank:{dist.get_rank()} ************* Replaced ' + str(unused_count) + f' codebooks *************')
            self.codebooks_used[:] = 0.0

    def inference(self, input_data_first, input_data_last, mask_last=None, user_action_token_num=None, condn_first=True):
        ...
        # print(f"distances: {distances.shape}, {distances.min()}, {distances.max()}, {distances.device}")
        min_indices = torch.argmin(distances, dim=1)
        # print(f"min_indices: {min_indices.shape}, {min_indices.min()}, {min_indices.max()}, {min_indices.device}")
        
        if user_action_token_num is not None:
            if type(user_action_token_num) == list:
                min_indices = torch.tensor(user_action_token_num, device=self.codebooks.device)
            else:                
                min_indices = torch.tensor([[user_action_token_num]], device=self.codebooks.device).repeat(input_data.shape[0], 1)
    ...
    def codebook_reinit(self):
        raise NotImplementedError("Codebook reinitialization is not implemented yet.")
