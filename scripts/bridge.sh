...
CUDA_VISIBLE_DEVICES=${gpu_id} python simpler_env/main_inference.py --policy-model ${policy_model} --ckpt-path ${ckpt_path} --action-ensemble-temp ${action_ensemble_temp} --logging-dir ${logging_dir} \
  ...
  --robot-init-rot-quat-center 0 0 0 1 --robot-init-rot-rpy-range 0 0 1 0 0 1 0 0 1 --unnorm-key ${unnorm_key} --traj_length ${traj_len} --num_rollouts ${num_rollouts} --window_size ${window_size} \
  --temperature 1.0;
